...
heroImage: "https://cdn.siliconwit.com/education/robotics/trajectory-planning-motion-control.png"
---
...
flowchart TD
    subgraph Joint-Space
        A1[q_start] --> B1[Interpolate\neach joint\nindependently] --> C1[q_goal]
    end
    subgraph Task-Space
        A2[x_start] --> B2[Interpolate\nCartesian pose\n+ IK at each step] --> C2[x_goal]
    end
    D{Which to use?}
    D -- Path shape does not matter --> Joint-Space
    D -- Path shape is critical --> Task-Space
```
...
**How it works:** Given start pose $\mathbf{x}_0$ and goal pose $\mathbf{x}_f$ in Cartesian space, interpolate position (and orientation using quaternion SLERP, covered in [Orientation and Quaternions](./orientation-quaternions)), then solve inverse kinematics at each time step.
...
```text
  ...
  If distance is too short to reach v_max,
  the cruise phase vanishes and the profile
  becomes triangular (accel then decel).
```
...
<Card title="S-Curve Profiles: Adding a Jerk Limit" icon="information">
A trapezoidal profile has discontinuous acceleration at four places (start of accel, start of cruise, start of decel, end of decel). Each jump is an instantaneous step in acceleration, which means **infinite jerk** ($\dddot{q}$). For delicate payloads (electronics, surgical tools, full cups of liquid) this is unacceptable: the sudden acceleration change feels like a hammer blow at the attachment.

The fix is the **S-curve** (or seven-segment) profile, which adds constant-jerk ramps to each corner of the trapezoid. The velocity curve has a characteristic S shape at each end. You specify a third limit $j_{\max}$ alongside $v_{\max}$ and $a_{\max}$, and the profile smoothly ramps acceleration up to $a_{\max}$, cruises at constant $a$, ramps back to zero, cruises at $v_{\max}$, and mirrors the pattern for deceleration. Most industrial motion controllers (Siemens SIMATIC, Rockwell Kinetix, Beckhoff TwinCAT) ship with S-curve profile generators; implement one yourself only if you need custom behavior.
</Card>

<Card title="Beyond Trapezoidal: TOPP for Time-Optimal Paths" icon="rocket">
The trapezoidal profile is time-optimal **for one joint at a time**. For a full robot following a Cartesian path, the binding constraint at each point along the path is usually a different joint (or a different axis of the task frame), and the joint velocity / acceleration / torque limits are themselves configuration-dependent. Computing a true time-optimal profile under all these constraints simultaneously is a classical problem solved by **Time-Optimal Path Parameterization** (TOPP).

The current industrial standard is **TOPP-RA** (Pham & Pham, 2017): it discretizes the geometric path and solves a sequence of small linear programs to find the fastest possible time parameterization under joint-velocity, joint-acceleration, and optionally joint-torque bounds. You will find it in [toppra (Python)](https://hungpham2511.github.io/toppra/), [Drake](https://drake.mit.edu/), and as the `contopp` option in MATLAB's Robotics System Toolbox. If the question "how do I make this trajectory 10 percent faster without exceeding limits?" comes up, this is the tool.
</Card>
...
stateDiagram-v2
    ...
    MoveToApproach: Move to approach\n(joint-space)
    Descend: Descend to object\n(task-space, vertical)
    Grasp: Grasp\n(dwell 200-500 ms)
    Lift: Lift\n(task-space, vertical)
    Transport: Transport\n(joint-space, trapezoidal)
    Lower: Lower to placement\n(task-space, vertical)
    Release: Release\n(dwell 200-500 ms)
...
For task-space segments (descent, lift, placement), we generate a straight line in Cartesian space and convert to joint trajectories using the Jacobian, introduced in [Velocity Kinematics and the Jacobian](./velocity-kinematics-jacobian).
...
<Card title="What If the Cartesian Path Is Infeasible?" icon="warning">
A straight line in Cartesian space can demand joint velocities above actuator limits, especially near the workspace boundary or near a singularity where small Cartesian motion requires large joint motion. Before executing a task-space trajectory, validate it by sweeping along the path at your planned speed, computing $\dot{\mathbf{q}} = J^{\dagger} \dot{\mathbf{x}}$ at each step, and checking that every joint stays within `[-v_max, v_max]` and every acceleration stays within `[-a_max, a_max]`.

If any step violates limits, you have three practical options:
1. **Slow the trajectory globally**: reduce the Cartesian cruise velocity until the worst joint is within limits. Easy to implement, costs time.
2. **Reparameterize using TOPP**: find the time-optimal speed profile along the fixed geometric path. Best if the path shape must be preserved.
3. **Replan the path**: move the via points away from the singular region, sacrificing a straight line for a curved but feasible motion. Best when the Cartesian shape is flexible.

Silently commanding unreachable velocities will either produce a tracking error (the robot falls behind) or trip the controller's safety limits and halt the motion mid-task.
</Card>
