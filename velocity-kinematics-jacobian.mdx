...
heroImage: "https://cdn.siliconwit.com/education/robotics/velocity-kinematics-jacobian.png"
---
...
<Card title="Body vs Spatial Jacobian: Two Ways to Express Velocity" icon="information">
When you move to 3D kinematics (and to any ROS or MoveIt codebase) you will encounter **two flavors** of the geometric Jacobian, differing only in the frame where end-effector twist is expressed:

- **Spatial (world-frame) Jacobian $J_s$**: end-effector twist $\mathcal{V}_s = J_s \dot{\mathbf{q}}$ is in base / world coordinates. Natural for tracking a world-fixed target.
- **Body (tool-frame) Jacobian $J_b$**: twist $\mathcal{V}_b = J_b \dot{\mathbf{q}}$ is in end-effector coordinates. Natural for tool-relative motion ("push 5 mm along the tool $z$").

They are related by the $6 \times 6$ **adjoint** of the tool-to-base transform: $J_s = [\text{Ad}_{T_{sb}}] J_b$ and $J_b = [\text{Ad}_{T_{bs}}] J_s$. This is not simply $J_s R$, it includes position cross-product terms through the full adjoint matrix. Pick one convention per project and stick to it; silently mixing spatial and body twists is a common source of sign errors in 6-DOF control. Modern Robotics (Lynch & Park) and the `modern_robotics` Python library default to the body Jacobian; MoveIt and KDL expose both.

Everything in this lesson's 2R example is planar, so the distinction collapses (the rotation between base and tool frame only changes orientation within the plane). It matters the moment you extend to 3D.
</Card>
...
<Card title="Practical Singularity Thresholds" icon="warning">
Unit-dependent measures like $|\det(J)|$ are fine for a single robot with known link lengths, but they do not transfer directly between robots of different sizes. For portable thresholds, use the dimensionless **condition number** $\kappa = \sigma_{\max} / \sigma_{\min}$ from the SVD. By convention:

- $\kappa < 10$: isotropic, safe for full-speed operation
- $\kappa \approx 100$: getting non-isotropic, one direction noticeably slower
- $\kappa \approx 1000$: near-singular, activate damped least-squares
- $\kappa > 10000$: effectively singular, reduce speed or replan

For collaborative robots the damping typically ramps in around $\kappa \approx 100$ so the transition is smooth. Alternatively, use the smallest singular value directly: activate damping when $\sigma_{\min} < \sigma_{\text{threshold}}$, a common default being 5 to 10 percent of the robot's maximum reachable velocity at isotropic configurations.
</Card>
...
:::note[Prefer SVD for Numerical Stability]
The formula above is pedagogically clear but numerically poor near singularities, because $JJ^T$ becomes ill-conditioned. The standard, numerically stable way to compute the pseudoinverse is via the SVD: if $J = U \Sigma V^T$, then $J^{\dagger} = V \Sigma^{\dagger} U^T$, where $\Sigma^{\dagger}$ inverts the nonzero singular values and leaves small ones as zero (or clips them with a damping floor). In NumPy, call `np.linalg.pinv(J)`, which uses SVD internally; the `rcond` argument tunes the singular-value cutoff.
:::
...
A concrete joint-limit avoidance example: for each joint, the gradient of a centering cost $H(\mathbf{q}) = \sum_i ((q_i - q_{i,\text{center}}) / (q_{i,\max} - q_{i,\min}))^2$ pushes that joint toward the middle of its range. Feed the negative gradient as $\mathbf{z}$ into the null-space term:

```python
def joint_limit_gradient(q, q_min, q_max):
    """Gradient of the quadratic centering cost."""
    center = (q_min + q_max) / 2
    span   = (q_max - q_min) / 2
    return 2 * (q - center) / span**2

def redundant_ik_velocity(J, dx_desired, q, q_min, q_max, k_sec=1.0):
    """Primary: track dx_desired. Secondary: center joints in their limits."""
    J_pinv = np.linalg.pinv(J)
    z      = -k_sec * joint_limit_gradient(q, q_min, q_max)
    null   = (np.eye(len(q)) - J_pinv @ J) @ z
    return J_pinv @ dx_desired + null
```

The primary task (end-effector velocity) is untouched because null-space motion is projected out of the task direction. Pick $k_{\text{sec}}$ small enough that the secondary term does not dominate the joint budget.
...
Next, in [Trajectory Planning and Motion Control](./trajectory-planning-motion-control), we will build on the Jacobian to plan smooth trajectories in both joint space and task space, using polynomial interpolation and velocity profiles to execute pick-and-place operations with the collaborative robot.
