---
title: "Robot Arm Geometry and Configuration"
...
heroImage: "https://cdn.siliconwit.com/education/robotics/robot-arm-geometry-configuration.png"
---
...
:::note[Two Flavors of DH Exist]
When you import DH tables from a real robot, you will quickly discover there are **two DH conventions** in use: the **classical** (original 1955) version and the **modified** (Craig 1986) version. They differ in where each link frame is attached (at the end of link $i$ versus at joint $i$) and in the order of the elementary transforms. Parameters with the same name can refer to different physical quantities. Manufacturers are inconsistent: KUKA, ABB, and Universal Robots publish tables that are not directly interchangeable. Always check which convention a table uses before feeding it into your FK code, otherwise you will get silently wrong poses. The next lesson picks one convention and uses it consistently.
:::
...
:::note[Parallel vs Serial Kinematics]
Unlike SCARA, articulated, and Cartesian arms, a delta robot is a **parallel mechanism**: it has **closed kinematic loops** between base and end-effector. This changes the DOF analysis fundamentally. The simple "sum the joint DOFs" rule no longer applies; you must use Grubler's formula for closed chains (a delta has around 9 joints but only 3 DOF because most are passive). Forward and inverse kinematics are also inverted compared to serial arms: IK is easy (solve each leg independently), FK is hard (solve a system of three simultaneous nonlinear equations). This is why parallel-robot controllers look so different from serial ones. See [Kinematic Joints and Degrees of Freedom](/education/spatial-mechanics/kinematic-joints-degrees-freedom/) for the full DOF counting machinery.
:::
...
## Python Workspace Visualization
...
**What comes next:** [Forward and Inverse Kinematics](./forward-inverse-kinematics) formalizes the relationship between joint angles and end-effector position using the Denavit-Hartenberg convention, building the mathematical framework for precise robot motion control.
